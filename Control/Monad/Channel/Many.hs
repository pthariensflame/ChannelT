...
FreeT a >+< FreeT b = FreeT $ do x <- a
                                 ...
                                 case (x, y) of
                                   (Pure v, _) -> Pure v
                                   (_, Pure v) -> Pure v
                                   (Free (SyncChannel (LiftSMulti s) o i), _) -> runFreeT $ liftS (syncOn s o) >>= \v -> i v >+< return b
                                   (_, Free (SyncChannel (LiftSMulti s) o i)) -> runFreeT $ liftS (liftS (syncOn s o)) >>= \v -> return a >+< i v
